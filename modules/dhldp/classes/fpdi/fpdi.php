...
class FPDI extends FPDFTPL
{
    ...
    protected function _getPdfParser($filename)
    {
        ...
        return new FPDIPDFParser($filename);
    }
        ...
        foreach ($this->parsers as $filename => $p) {
            ...
            while (($n = key($this->_objStack[$filename])) !== null) {
                try {
                    ...
                    $nObj = array(PDFParser::TYPE_OBJECT, PDFParser::TYPE_NULL);
                }

                $this->_newobj($this->_objStack[$filename][$n][0]);

                if ($nObj[0] == PDFParser::TYPE_STREAM) {
    ...
    protected function _putformxobjects()
    {
        ...
        foreach ($this->_tpls as $tplIdx => $tpl) {
            ...
            if (is_subclass_of($this, 'TCPDF')) {
                ...
                $this->_out('/Length ' . Tools::strlen($buffer, 'latin1') . ' >>');
                $this->_out("stream\n" . $buffer . "\nendstream");
            } else {
                $this->_out('/Length ' . Tools::strlen($buffer, 'latin1') . ' >>');
    ...
    public function _newobj($objId = false, $onlyNewObj = false)
    {
        ...
        //Begin a new object
        if (!$onlyNewObj) {
            $this->offsets[$objId] = is_subclass_of($this, 'TCPDF') ? $this->bufferlen : Tools::strlen(
                $this->buffer,
                'latin1'
            );
    ...
    protected function _writeValue(&$value)
    {
        ...
        switch ($value[0]) {
            case PDFParser::TYPE_TOKEN:
                $this->_straightOut($value[1] . ' ');
                break;
            case PDFParser::TYPE_NUMERIC:
                //no break
            case PDFParser::TYPE_REAL:
            ...
            case PDFParser::TYPE_OBJREF:
                ...
                $this->_out($objId . ' 0 R');
                break;
            case PDFParser::TYPE_STRING:
                // A string.
                $this->_straightOut('(' . $value[1] . ')');
                break;
            case PDFParser::TYPE_STREAM:
                ...
                break;
            case PDFParser::TYPE_HEX:
                $this->_straightOut('<' . $value[1] . '>');
                break;
            case PDFParser::TYPE_BOOLEAN:
                $this->_straightOut($value[1] ? 'true ' : 'false ');
                break;
            case PDFParser::TYPE_NULL:
                // The null object.
        ...
        if (!is_subclass_of($this, 'TCPDF')) {
            ...
            if ($this->state == 2) {
                if ($this->inxobj) {
                    ...
                    if ((!$this->InFooter) && isset($this->footerlen[$this->page]) && ($this->footerlen[$this->page] > 0)) {
                        // puts data before page footer
                        $pagebuff = $this->getPageBuffer($this->page);
                        $page = Tools::substr($pagebuff, 0, -$this->footerlen[$this->page], 'latin1');
                        $footer = Tools::substr($pagebuff, -$this->footerlen[$this->page], false, 'latin1');
                        $this->setPageBuffer($this->page, $page . $s . $footer);
                        // update footer position
                        $this->footerpos[$this->page] += Tools::strlen($s, 'latin1');
                    } else {
