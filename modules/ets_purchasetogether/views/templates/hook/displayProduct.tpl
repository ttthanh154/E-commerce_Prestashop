...
{if isset($configs.ETS_PT_DISPLAY_TYPE) && $configs.ETS_PT_DISPLAY_TYPE == 1}
    {if isset($purchase_togethers) && $purchase_togethers && count($purchase_togethers) != $alldisabled}
    	...
    	<div class="button-container {if count($purchase_togethers) == $alldisabled}disabled{/if}">
    		<a class="button ets_ajax_add_to_cart_button btn btn-default" href="{$ajax_cart|escape:'html':'UTF-8'}" rel="nofollow" title="{l s='Add all products to cart' mod='ets_purchasetogether'}">
    			<span>{l s='Add all products to cart' mod='ets_purchasetogether'}</span>
    		</a>
    	</div>
    {/if}
{else}
