...
if(Tools::isSubmit('token') && Tools::getValue('token')!=Tools::getAdminTokenLite('AdminModules'))
    die(Tools::jsonEncode(array('error'=>$module->l('Token is not valid'))));
if(Tools::getValue('idexport'))
    die(Tools::jsonEncode($module->synchronizeMailchimpList((int)Tools::getValue('idexport'))));
else
    die(Tools::jsonEncode($module->synchronizeAllMailchimpLists()));